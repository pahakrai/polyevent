...
import { chatStream, type ChatResult } from '@/lib/knowledge-api';
...
export default function ChatPage() {
    ...
    try {
      await chatStream(
        ...
        (token) => {
          ...
          setMessages((prev) => {
            const updated = [...prev];
            if (updated[placeholderIdx] && updated[placeholderIdx].role === 'assistant') {
              updated[placeholderIdx] = { ...updated[placeholderIdx], content: assistantContent };
            } else {
              updated[placeholderIdx] = { role: 'assistant', content: assistantContent };
            }
            return updated;
          });
        },
        (sources) => {
          ...
          setMessages((prev) => {
            ...
            if (updated[placeholderIdx]) {
              updated[placeholderIdx] = { ...updated[placeholderIdx], sources };
            }
            return updated;
          });
        },
      ...
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Sign in to ask questions about company policies and procedures.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-6rem)] max-w-3xl flex-col px-4">
      <div className="mb-4">
        ...
        {messages.length === 0 ? (
          <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
            <div className="max-w-md text-center space-y-2">
              <p className="font-medium">Ask anything about our knowledge base</p>
              <p>
                Questions about refunds, venue policies, booking rules —
                our documents have the answers.
              </p>
            </div>
          </div>
        ) : (
